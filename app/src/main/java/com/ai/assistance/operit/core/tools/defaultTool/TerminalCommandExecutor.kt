    ...
    fun invoke(tool: AITool): ToolResult {
        return runBlocking {
            try {
                ...
                val timeout = tool.parameters.find { param -> param.name == "timeout_ms" }?.value?.toLongOrNull() ?: 30000L
                val withSessionId = tool.parameters.find { param -> param.name == "session_id" }?.value

                // 获取会话
                val session = if (!withSessionId.isNullOrEmpty()) {
                    TerminalSessionManager.sessions.find { s: com.ai.assistance.operit.ui.features.toolbox.screens.terminal.model.TerminalSession -> s.id == withSessionId }
                } else {
                    // 如果没有指定会话ID，使用当前活动会话，或者创建一个新会话
                    TerminalSessionManager.getActiveSession() ?: TerminalSessionManager.createSession("命令执行会话")
                }
