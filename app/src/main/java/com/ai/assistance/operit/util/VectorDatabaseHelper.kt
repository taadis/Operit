...
class VectorDatabaseHelper(private val context: Context) {
    ...
    class SimpleVectorIndex<TId, TItem : Item<TId, FloatArray>> {
        ...
        fun findNearest(queryVector: FloatArray, k: Int): List<SearchResult<TItem, Float>> {
            // 计算所有项目与查询向量的余弦相似度
            val results = items.values.map { item ->
                ...
                searchResult as SearchResult<TItem, Float>
            }
    ...
    fun generateVector(record: ProblemLibraryTool.ProblemRecord): FloatArray {
        // 构建文本表示，权重不同字段
        val textContent = StringBuilder().apply {
            // 摘要权重最高
    ...
    suspend fun addRecord(record: ProblemLibraryTool.ProblemRecord) {
        try {
            ...
            // 将向量序列化为ByteArray
            val vectorData = ByteBuffer.allocate(VECTOR_SIZE * 4).apply {
    ...
    // 搜索相似记录
    suspend fun searchSimilar(query: String, maxResults: Int = 10): List<ProblemLibraryTool.ProblemRecord> {
        try {
            ...
            val finalResults = uuids.mapNotNull { uuid -> 
    ...
    fun loadIndex(): Boolean {
        try {
            ...
            ObjectInputStream(FileInputStream(indexFile)).use { ois ->
                ...
                // 将加载的索引中的项目添加到当前索引
                vectorMap.forEach { (id, vector) ->
                    index.add(ProblemItem(id, vector))
                }
            }
        ...
        try {
            ...
            for (entity in allProblems) {
                ...
                if (vectorData != null) {
                    ...
                    val vector = ByteBuffer.wrap(vectorData).order(ByteOrder.LITTLE_ENDIAN).let { buffer ->
                    ...
                    // 更新数据库中的向量数据
                    val newVectorData = ByteBuffer.allocate(VECTOR_SIZE * 4).apply {
                        order(ByteOrder.LITTLE_ENDIAN)
                        for (value in vector) {
                            putFloat(value)
                        }
