...
class ExpressionParser(private val expression: String) {
    ...
    private fun parseAssignment(): ExpressionNode {
        if (currentTokenType == TokenType.IDENTIFIER) {
            ...
            if (nextChar == '=') {
                val followingChar = if (position + 1 < expression.length) expression[position + 1] else ' '
            ...
            TokenType.IDENTIFIER -> {
                ...
                if (currentToken == "(") {
                    ...
                    if (identifier.equals("convert", ignoreCase = true) && args.size >= 3) {
                        val fromUnit = (args[1] as? VariableNode)?.name ?: args[1].evaluate().toString()
                        val toUnit = (args[2] as? VariableNode)?.name ?: args[2].evaluate().toString()
    ...
    private fun parseTemplate(): ExpressionNode {
        ...
        while (currentTokenType == TokenType.TEMPLATE_MIDDLE || currentTokenType == TokenType.TEMPLATE_END) {
    ...
    private fun nextToken() {
        ...
        when {
            c.isDigit() || (c == '.' && position + 1 < expression.length && expression[position + 1].isDigit()) -> {
                scanNumber()
            }
            c.isLetter() || c == '_' -> {
                scanIdentifier()
