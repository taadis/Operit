...
class AdbCommandExecutor {
    companion object {
        ...
        /**
         * 检测命令是否包含需要shell解释的特殊操作符 这个方法更准确地检测操作符，避免误判命令参数中包含的特殊字符
        ...
        /** 直接执行不包含特殊操作符的普通命令 */
        private suspend fun executeCommandDirect(command: String): CommandResult =
                withContext(Dispatchers.IO) {
                    ...
                    try {
                        ...
                        val stdout =
                                if (inputStream != null) {
                                    retryOperation {
                                        val stdoutStream =
                                                FileInputStream(inputStream.fileDescriptor)
                        ...
                        val stderr =
                                if (errorStream != null) {
                                    retryOperation {
                                        val stderrStream =
                                                FileInputStream(errorStream.fileDescriptor)
                        ...
                        // 返回结果
                        return@withContext CommandResult(exitCode == 0, stdout, stderr, exitCode)
        ...
        private suspend fun executeWithShell(command: String): CommandResult =
                withContext(Dispatchers.IO) {
                    ...
                    try {
                        ...
                        Log.d(
                                TAG,
                                "Enhanced shell command: ${shellArgs.joinToString(", ", "[", "]")}"
                        )
                        ...
                        Log.d(
                                TAG,
                                "Shell execution result: exitCode=$exitCode, success=${exitCode == 0}, stdout length=${stdout.length}, stderr length=${stderr.length}"
                        )
                        ...
                        return@withContext CommandResult(success, stdout, stderr, exitCode)
                    } catch (e: Exception) {
