...
class FFmpegConvertToolExecutor(private val context: Context) : ToolExecutor {
    ...
    override fun invoke(tool: AITool): ToolResult {
        ...
        return try {
            ...
            if (ReturnCode.isSuccess(returnCode)) {
                ...
                val ffmpegResult = if (mediaInfo != null) {
                    val videoStreams = mediaInfo.streams
                        ...
                        .map { stream ->
                            FFmpegResultData.StreamInfo(
                                ...
                                frameRate = null // We'll get this from FFprobe if needed
                            )
                        }
                        .toMutableList()
                    ...
                    val audioStreams = mediaInfo.streams
                        ...
                        .map { stream ->
                            FFmpegResultData.StreamInfo(
                                ...
                                codecType = stream.type ?: "unknown",
                                codecName = stream.codec ?: "unknown",
                                sampleRate = null, // We'll get this from FFprobe if needed
                                channels = null // We'll get this from FFprobe if needed
                            )
                        }
                        .toMutableList()
                    ...
                    if (ffprobeInfo != null) {
                        ...
                        ffprobeInfo.streams.forEach { probeStream ->
                            when (probeStream.type) {
                                "video" -> {
                                    val index = videoStreams.indexOfFirst { it.index == probeStream.index?.toInt() }
                                    if (index != -1) {
                                        val stream = videoStreams[index]
                                        videoStreams[index] = stream.copy(
                                            frameRate = probeStream.allProperties?.get("r_frame_rate")?.toString()
                                        )
                                    }
                                }
                                "audio" -> {
                                    val index = audioStreams.indexOfFirst { it.index == probeStream.index?.toInt() }
                                    if (index != -1) {
                                        val stream = audioStreams[index]
                                        audioStreams[index] = stream.copy(
                                            sampleRate = probeStream.allProperties?.get("sample_rate")?.toString(),
                                            channels = probeStream.allProperties?.get("channels")?.toString()?.toIntOrNull()
                                        )
                                    }
                                }
                            }
                    ...
                    FFmpegResultData(
                        ...
                        mediaInfo = FFmpegResultData.MediaInfo(
    ...
    override fun validateParameters(tool: AITool): ToolValidationResult {
        ...
        if (outputPath.isNullOrEmpty()) {
            return ToolValidationResult(
                valid = false,
                errorMessage = "必须提供output_path参数"
            )
        }
