...
class FileSystemTools(private val context: Context) {
    ...
    suspend fun copyFile(tool: AITool): ToolResult {
        val sourcePath = tool.parameters.find { it.name == "source" }?.value ?: ""
        val destPath = tool.parameters.find { it.name == "destination" }?.value ?: ""
        val recursive = tool.parameters.find { it.name == "recursive" }?.value?.toBoolean() ?: true // 默认为true以支持目录复制
        ...
        return try {
            ...
            if (result.success) {
                // 验证复制是否成功
                val verifyResult = AdbCommandExecutor.executeAdbCommand("test -e '$destPath' && echo 'exists' || echo 'not exists'")
                if (verifyResult.stdout.trim() != "exists") {
                    return ToolResult(
                            toolName = tool.name,
                            success = false,
                            result = FileOperationData(
                                    ...
                                    path = sourcePath,
                                    successful = false,
                                    details = "Copy command completed but destination does not exist"
                            ),
                ...
                return ToolResult(
                        ...
                        result = FileOperationData(
                                ...
                                path = sourcePath,
                                successful = true,
                                details = "Successfully copied ${if (isDirectory) "directory" else "file"} $sourcePath to $destPath"
                        ),
                        error = ""
                )
            } else {
    ...
    suspend fun findFiles(tool: AITool): ToolResult {
        ...
        return try {
            ...
            val command = "find ${if(path.endsWith("/")) path else "$path/"} $depthOption $searchOption $patternForCommand"
    ...
    suspend fun zipFiles(tool: AITool): ToolResult {
        ...
        return try {
            ...
            // Create temporary file for processing - using external files directory for better
            // permissions
            ...
            try {
                ...
                if (isDirectory) {
                    ...
                    val listResult =
                            AdbCommandExecutor.executeAdbCommand("find '$sourcePath' -type f")
    ...
    suspend fun unzipFiles(tool: AITool): ToolResult {
        ...
        return try {
            ...
            // Create temporary files for processing - using external files directory for better
            // permissions
            val tempDir = context.getExternalFilesDir(null) ?: context.cacheDir
            val tempZipFile = File(tempDir, "temp_zip_${System.currentTimeMillis()}.zip")
