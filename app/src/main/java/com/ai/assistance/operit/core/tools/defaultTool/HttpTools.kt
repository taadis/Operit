...
class HttpTools(private val context: Context) {
    ...
    /** 发送HTTP请求 支持GET, POST, PUT, DELETE方法，并可自定义请求头和请求体 */
    ...
    private fun extractTextFromHtml(html: String, contentType: String): String {
        return try {
            if (contentType.contains("html", ignoreCase = true) ||
                ...
                val mainContent =
                        doc.select(
                                        "article, main, div.content, div.main, div.article, div[class*=content], div[class*=article]"
                                )
                                .text()
    ...
    private fun ultraSummarizeContent(content: String, maxLength: Int): String {
        ...
        if (paragraphCount <= 3) {
            ...
            if (paragraphCount > 1) {
                ...
                val lastParaLength =
                        minOf(lastParagraph.length, (remainingLength - 30).toInt().coerceAtLeast(0))
            ...
            val estimatedSampleLength =
                    ((remainingLength - 40) / samplesToTake).toInt().coerceAtLeast(50)
            ...
            for (i in 1 until paragraphCount - 1 step sampleInterval) {
                ...
                val sample =
                        if (paragraph.length <= sampleLength) {
                            paragraph
                        } else {
    ...
    private fun createJsonObjectSummary(jsonObject: JSONObject, maxLength: Int): String {
        ...
        if (totalKeys > keysPerSection && charsRemaining > 0) {
            ...
            for (key in lastKeys) {
                ...
                val valueSummary = getJsonValueSummary(jsonObject, key, 100)
                val entryString =
                        "  \"$key\": $valueSummary${if (key != lastKeys.last()) "," else ""}\n"
    ...
    /** 获取JSON数组元素的摘要表示 */
    private fun getJsonArrayElementSummary(
            jsonArray: JSONArray,
            index: Int,
            maxLength: Int
    ): String {
