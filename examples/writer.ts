...
{
  name: writer
  description: 提供高级文件编辑和读取功能，支持分段编辑、差异编辑、行号编辑以及高级文件读取操作，如分段读取、正则表达式匹配、函数块提取等。
...
const writer = (function () {
     ...
     */
    async function read_lines(params: { file_path: string, start_line: number, end_line: number }) {
        // 定义参数类型
        const paramTypes = {
            start_line: 'string',
            end_line: 'string'
     ...
     */
    async function read_regex_matches(params: { file_path: string, regex: string, include_line_numbers?: boolean }) {
        ...
        try {
            ...
            for (let i = 0; i < lines.length; i++) {
                const line = lines[i];
                const lineMatches: Array<{ text: string, index: number, groups: string[] }> = [];
                let match;
     ...
     */
    async function edit_lines(params: { file_path: string, start_line: number, end_line: number, content: string }) {
     ...
     */
    async function insert_at_line(params: { file_path: string, line_number: number, content: string, before?: boolean }) {
        // 定义参数类型
        const paramTypes = {
            line_number: 'string',
            before: 'string'
     ...
     */
    async function extract_function(params: { file_path: string, function_name: string, include_line_numbers?: boolean }) {
        // 定义参数类型
        const paramTypes = {
     ...
     * @returns {Promise<Object>} 包含编辑结果的Promise
     */
    async function diff_edit(params: { file_path: string, original_text: string, new_text: string }) {
     ...
     */
    async function batch_replace(params: { file_path: string, replacements: Array<{ pattern: string, replacement: string }>, use_regex?: boolean }) {
        // 定义参数类型
        const paramTypes = {
            replacements: 'string',
            use_regex: 'string'
     ...
     */
    async function process_large_file(params: { file_path: string, chunk_size?: number, start_line?: number, max_chunks?: number }) {
        // 定义参数类型
        const paramTypes = {
            chunk_size: 'string',
            start_line: 'string',
     ...
     * @returns {Promise<Object>} 包含提取结果的Promise
     */
    async function extract_code_block(params: { file_path: string, block_start: string, block_end: string, context_lines?: number }) {
        // 定义参数类型
        const paramTypes = {
     ...
     */
    async function find_nested_structures(params: { file_path: string, open_pattern: string, close_pattern: string, max_depth?: number }) {
        // 定义参数类型
        const paramTypes = {
            max_depth: 'string'
        };
        ...
        try {
            ...
            for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
                ...
                while ((match = closeRegex.exec(line)) !== null) {
                    if (stack.length > 0) {
                        // 找到配对的开始标识
                        const openMarker = stack.pop() as StructureMarker;
    ...
    return {
        read_lines: async (params) => await writer_wrap(
            read_lines,
            params,
            "成功读取指定行范围",
            "读取指定行范围失败"
        ),
        read_regex_matches: async (params) => await writer_wrap(
            read_regex_matches,
            params,
            "成功读取正则表达式匹配内容",
            "读取正则表达式匹配内容失败"
        ),
        edit_lines: async (params) => await writer_wrap(
            edit_lines,
            params,
            "成功编辑指定行范围",
            "编辑指定行范围失败"
        ),
        insert_at_line: async (params) => await writer_wrap(
            insert_at_line,
            params,
            "成功在指定行插入内容",
            "在指定行插入内容失败"
        ),
        extract_function: async (params) => await writer_wrap(
            extract_function,
            params,
            "成功提取函数",
            "提取函数失败"
        ),
        diff_edit: async (params) => await writer_wrap(
            diff_edit,
            params,
            "成功差异编辑文件",
            "差异编辑文件失败"
        ),
        batch_replace: async (params) => await writer_wrap(
            batch_replace,
            params,
            "成功批量替换文件内容",
            "批量替换文件内容失败"
        ),
        process_large_file: async (params) => await writer_wrap(
            process_large_file,
            params,
            "成功处理大文件",
            "处理大文件失败"
        ),
        extract_code_block: async (params) => await writer_wrap(
            extract_code_block,
            params,
            "成功提取代码块",
            "提取代码块失败"
        ),
        find_nested_structures: async (params) => await writer_wrap(
            find_nested_structures,
            params,
            "成功查找嵌套结构",
            "查找嵌套结构失败"
        )
