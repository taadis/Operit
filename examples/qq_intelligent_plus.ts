...
async function reply(params: { message: string, target: string }) {
  // 提取参数
...
async function find_user(params: { search_term: string, search_type: string }) {
  // 提取参数
  ...
  try {
    ...
    if (searchType === "contacts" || searchType === "both") {
      ...
      // 输入搜索内容
      await Tools.UI.setText(searchTerm);
      ...
      // 获取搜索结果界面
      const contactSearchResults = await Tools.UI.getPageInfo();
    }
