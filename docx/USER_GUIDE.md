...
> 考虑到手机环境的特殊性，要完整、稳定地配置所有MCP（Meta-Call-Package）所需的环境是相当有挑战性的。因此，直接调用MCP可能会遇到较多困难。
> 
> 目前，应用内确认可用的MCP主要有 `12306` 和 `fetch`。但需要注意的是，`fetch` 的搜索效果和稳定性均不如我们深度优化的 `various_search` 拓展包。
> 
